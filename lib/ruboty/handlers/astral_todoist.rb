...
module Ruboty
  module Handlers
    ...
    class AstralTodoist < Base
      on /todoist tasks list\s+(?<date>.*)\s+(?<project_name>.*)\s+(?<sort_type>.*)/, name: 'todoist_tasks', description: 'list tasks'
      on /todoist tasks first\s+(?<date>.*)\s+(?<project_name>.*)\s+(?<sort_type>.*)/, name: 'todoist_tasks_first', description: 'take first task'
      on /todoist tasks complete_first\s+(?<date>.*)\s+(?<project_name>.*)\s+(?<sort_type>.*)/, name: 'todoist_tasks_complete_first', description: 'complete first task'
